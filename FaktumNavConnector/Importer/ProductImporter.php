...
use Immerce\FaktumNavConnector\Model\ProductCompareStatus;
...
class ProductImporter extends AbstractImporter
{
    ...
    private ProductCompareStatus $productCompareStatus;
    ...
    public function __construct(
        ...
        ProductCompareStatus $productCompareStatus,
        ...
        $this->productCompareStatus = $productCompareStatus;
    ...
    {
        ############### Start to check disable product ##############
        $this->disableAllProducts->loadXml($data)->execute();
        ############### End to check disable product ##############
...
//                $product = $this->product->load($exitId);
                $product = $this->productFactory->create()->load($exitId);
                $product->setStoreId($storeId);

                /** start compare product update or not */
                $this->productCompareStatus->setProduct($product);
                $this->productCompareStatus->setXmlItems($this->currentItem);
...
//                $websiteId = $this->storeManager->getWebsite()->getId(); //can't use in CLI command mode.
                ...
                if (is_numeric($exitId)) {
                    ...
                    if ($this->isValid($this->currentItem,'name', 'string')) {
                        $product->setName($this->currentItem['name']);
                    }
                    if ($this->isValid($this->currentItem,'description', 'string')) {
                        $product->setDescription($this->currentItem['description']);
                    }
                    if ($this->isValid($this->currentItem,'short_description', 'string')) {
                        $product->setShortDescription($this->currentItem['short_description']);
                    }
                } else {
                ...
                if ($this->isValid($this->currentItem,'attributes', 'array')) {
                    ...
                    $product = $this->importAttributes($product, $this->currentItem['attributes']);
                }
                /** set stock */
                ...
                /** set images */
                //  ################ Disabled image check time query #################//
                if ($this->isValid($this->currentItem,'images', 'array')) {
                    $this->setImagesData($product, $this->currentItem['images']);
                }
    ...
    public function isValid(array $array, string $key, $type=null)
    {
        ...
        /** Check the content in a value had updated or not */
        if($status){
            /**
             * If have same data between xml and product db data.
             * We will return true, we don't need process again.
             */
            $status = $this->compareValues($key, $status);
        }

        return $status;
    }
    public function compareValues(string $attribute, bool $status): bool
    {
        if($this->productCompareStatus->hasProduct())
        {
            $status = ($this->productCompareStatus->hasSameData($attribute)) ? false : true;
        }
    ...
    protected function importPrices(Product $product, array $prices): Product
    {
        ...
        if(isset($prices['tax_class_name'])){
            $product = self::setTaxClassById($product, $prices['tax_class_name']);
        }
    ...
    /**
     * Set Tax class by tax name
     * @param Product $product
     * @param string $taxClassName
     * @return Product
     */
    private function setTaxClassById(Product $product, string $taxClassName): Product
    {
        $ObjectManager = ObjectManager::getInstance();
        $taxClassRepository = $ObjectManager->get(\Magento\Tax\Model\ClassModelFactory::class)->create();
        $taxClass = $taxClassRepository->load($taxClassName, 'class_name');
...
//        $websiteId = $this->storeManager->getWebsite()->getId();
    ...
    protected function importAttributes(Product $product, array $attributes): Product
    {
    ...
    {
        $stockItemData = [
            StockItemInterface::QTY => isset($stock['qty']) ? (int) $stock['qty'] : 0,
            StockItemInterface::USE_CONFIG_MANAGE_STOCK => true,
            StockItemInterface::IS_IN_STOCK => true,
            StockItemInterface::USE_CONFIG_MIN_SALE_QTY => true,
            StockItemInterface::USE_CONFIG_MAX_SALE_QTY => true,
        ];

        $product->setData('stock_data', $stockItemData);

        return $product;
    }

    private function clearProductImages(Product $product)
    {
        if($product->getId() > 0){
    ...
    protected function setImagesData(Product $product, array $images): Product
    {
        ...
        self::clearProductImages($product);
    ...
    protected function progressBar(int $done, int $total, string $info = '', $width = 50)
    {
        ...
        return sprintf("Product importer processing ... : %s%%[%s>%s]%s\r", $perc, str_repeat('=', $bar), str_repeat(' ', $width-$bar), $info);
    }
